...
logging.basicConfig(level=logging.INFO, filename='01_DataPreFix_New.log', filemode='a', format='%(asctime)s %(levelname)s: %(message)s')
...
# 將評論分詞
tokenized_reviews = data['ProcessedReview'].apply(lambda x: x.split())
logging.info("將評論分詞 完成")

# 使用Phrases找到常見的詞組
phrases = Phrases(tokenized_reviews, min_count=5, threshold=10)
bigram = Phraser(phrases)
bigram_reviews = [bigram[sent] for sent in tokenized_reviews]
logging.info("使用Phrases找到常見的詞組 完成")

# 訓練word2vec模型
model = Word2Vec(bigram_reviews, vector_size=100, window=5, min_count=5, workers=4, epochs=50)
...
for feature, similarity in similar_features:
...
# 保存成 CSV 檔案
data_prefix.to_csv("./01_DataPreFix_new.csv", index=False, encoding="utf-8")
